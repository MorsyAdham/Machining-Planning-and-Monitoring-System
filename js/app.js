...
import {
    flt as scheduleFlt,
    pad as schedulePad,
    clamp as scheduleClamp,
    getPartStatus as scheduleGetPartStatus,
    getPartStatusLabel as scheduleGetPartStatusLabel,
    syncDerivedPartStatus as scheduleSyncDerivedPartStatus,
    parseTime as scheduleParseTime,
    shiftDuration as scheduleShiftDuration,
    fmtTime as scheduleFmtTime,
    isWorkingDay as scheduleIsWorkingDay,
    genDates as scheduleGenDates,
    addWD as scheduleAddWD,
    ...
    scheduleParts as scheduleScheduleParts
} from './schedule.js';
import {
    RPT as EXPORT_RPT,
    reportHeaderRow as exportReportHeaderRow,
    reportDataRow as exportReportDataRow,
    reportGroupHeader as exportReportGroupHeader,
    buildWorksheet as exportBuildWorksheet,
    exportStamp as exportExportStamp,
    makePdfDoc as exportMakePdfDoc,
    downloadTemplate as exportDownloadTemplate,
    exportPartsPDF as exportRunPartsPDF,
    exportScheduleXLSX as exportRunScheduleXLSX,
    exportSchedulePDF as exportRunSchedulePDF,
    exportPartsXLSX as exportRunPartsXLSX,
    exportMachineUtilXLSX as exportRunMachineUtilXLSX,
    exportMachineUtilPDF as exportRunMachineUtilPDF,
    exportWeeklyPlanXLSX as exportRunWeeklyPlanXLSX,
    exportWeeklyPlanPDF as exportRunWeeklyPlanPDF,
    exportExecutiveSummaryXLSX as exportRunExecutiveSummaryXLSX,
    exportExecutiveSummaryPDF as exportRunExecutiveSummaryPDF
} from './exports.js';
import {
    loadProdSequenceData,
    saveProdSequenceData,
    loadSettingsData,
    saveSettingsData,
    loadShiftsData,
    upsertShiftData,
...
function flt(v) { return scheduleFlt(v); }
function pad(n, w) { return schedulePad(n, w); }
function clamp(v, lo, hi) { return scheduleClamp(v, lo, hi); }
...
function getPartStatus(part) { return scheduleGetPartStatus(part); }

function getPartStatusLabel(status) { return scheduleGetPartStatusLabel(status); }

function syncDerivedPartStatus(part) { return scheduleSyncDerivedPartStatus(part); }
...
function parseTime(t) { return scheduleParseTime(t); }

// Shift duration in hours (handles overnight: '16:00'→'00:00' = 8h)
function shiftDuration(shift) { return scheduleShiftDuration(shift); }

// Format time minutes → '08:00'
function fmtTime(mins) { return scheduleFmtTime(mins); }

// Is a given date a working day?
// Sun(0)–Thu(4) = yes | Fri(5) = NEVER | Sat(6) = only if saturday_working
function isWorkingDay(d, settings) { return scheduleIsWorkingDay(d, settings); }

// Generate `count` working dates from startStr
function genDates(startStr, count, settings) { return scheduleGenDates(startStr, count, settings); }

// Add `days` working days to startStr
function addWD(startStr, days, settings) { return scheduleAddWD(startStr, days, settings); }

// Count working days between two date strings (exclusive start, inclusive end)
function wdBetween(fromStr, toStr, settings) { return scheduleWdBetween(fromStr, toStr, settings); }

function fmtDate(d) { return scheduleFmtDate(d, MONTHS); }

// Local date string — avoids UTC shift (toISOString would give yesterday in UTC+2)
function localDateStr(d) { return scheduleLocalDateStr(d); }
...
═══════════════════════════════════════════════════════════════ */
function autoVehicleForUnit(part, ui, total) { return scheduleAutoVehicleForUnit(part, ui, total, BATTALION); }
...
function expandPart(part) {
    return scheduleExpandPart(part, {
        appSettings,
        battalion: BATTALION,
    });
}
...
function scheduleParts(parts, machines) {
    return scheduleScheduleParts(parts, machines, {
        appSettings,
        allShifts,
        battalion: BATTALION,
        prodSequence,
...
async function applySeqAndClose() {
    try {
        const { error } = await saveProdSequenceData(db, activeBuildingId, prodSequence);
...
async function loadProdSequence() {
    prodSequence = await loadProdSequenceData(db, activeBuildingId);
}
...
async function loadShifts() {
    allShifts = await loadShiftsData(db, activeBuildingId, FALLBACK_SHIFTS);
    renderShiftsConfig();
}
...
async function loadMachines() {
    currentMachines = await loadMachinesData(db, activeBuildingId);
    renderMachinesTable();
    populateGanttMachFilter();
}

async function saveMachine(m) {
    const { data, error } = await saveMachineData(db, m, activeBuildingId, currentMachines.length);
    if (error) throw error;
    return data;
}
...
function downloadTemplate() {
    return exportDownloadTemplate(showToast);
}
...
async function uploadFile(file) {
    ...
    try {
        ...
        await replacePartsData(db, activeBuildingId, withOrder);
...
async function loadAllPartsAndRebuild() {
    try {
        const data = await loadPartsData(db, activeBuildingId);
...
const RPT = EXPORT_RPT;

// Helper: styled header row
function _rh(cols) { return exportReportHeaderRow(cols); }

// Helper: data row with optional vehicle color + alternating bg
function _dr(cells, opts) { return exportReportDataRow(cells, opts); }

// Helper: group header row spanning all cols
function _grh(label, numCols) { return exportReportGroupHeader(label, numCols); }

// Helper: build a worksheet with column widths and row heights, auto-filter
function _buildWs(sheetData, colWidths, rowHeights) { return exportBuildWorksheet(XLSX, sheetData, colWidths, rowHeights); }
...
function getExportDeps() {
    return {
        ...
        localDateStr,
        mDailyForMachine,
        makePdfDoc: _makePdfDoc,
        reportHeaderRow: _rh,
        reportDataRow: _dr,
        reportGroupHeader: _grh,
        buildWorksheet: _buildWs,
    };
}

// ── PARTS LIST PDF ───────────────────────────────────────
function exportPartsPDF() {
    return exportRunPartsPDF(getExportDeps());
}

// ── PRODUCTION SCHEDULE XLSX ──────────────────────────────
function exportScheduleXLSX() {
    return exportRunScheduleXLSX(getExportDeps());
}

// ── PRODUCTION SCHEDULE PDF ───────────────────────────────
function exportSchedulePDF() {
    return exportRunSchedulePDF(getExportDeps());
}

// ── MACHINE UTILIZATION XLSX ─────────────────────────────
function exportMachineUtilXLSX() {
    return exportRunMachineUtilXLSX(getExportDeps());
}

// ── MACHINE UTILIZATION PDF ───────────────────────────────
function exportMachineUtilPDF() {
    return exportRunMachineUtilPDF(getExportDeps());
}

// ── WEEKLY PLAN XLSX ──────────────────────────────────────
function exportWeeklyPlanXLSX() {
    return exportRunWeeklyPlanXLSX(getExportDeps());
}

// ── WEEKLY PLAN PDF ───────────────────────────────────────
function exportWeeklyPlanPDF() {
    return exportRunWeeklyPlanPDF(getExportDeps());
}

// ── EXECUTIVE SUMMARY XLSX ───────────────────────────────
function exportExecutiveSummaryXLSX() {
    return exportRunExecutiveSummaryXLSX(getExportDeps());
}

// ── EXECUTIVE SUMMARY PDF ────────────────────────────────
function exportExecutiveSummaryPDF() {
    return exportRunExecutiveSummaryPDF(getExportDeps());
}

// ── SHARED PDF HELPERS ─────────────────────────────────────
function _makePdfDoc(title) {
    return exportMakePdfDoc(title, fmtDate);
}

function stamp() {
    return exportExportStamp();
}

// ── PARTS LIST XLSX ───────────────────────────────────────
function exportPartsXLSX() {
    return exportRunPartsXLSX(getExportDeps());
}
...
function showToast(msg, type) {
    showToastUi($, msg, type);
}
function showLoading(msg = 'Loading…') {
    showLoadingUi($, msg);
}
function hideLoading() { hideLoadingUi($); }
function setConn(ok) {
    isOK = ok;
    setConnUi($, ok);
}
...
async function updateDBStats() {
    try {
        const count = await countPartsData(db, activeBuildingId);
...
function applyTheme(t) {
    applyThemeUi($, t, THEME_KEY);
}
function toggleTheme() { applyTheme(document.documentElement.getAttribute('data-theme') === 'dark' ? 'light' : 'dark'); }
function loadTheme() { loadThemeUi(applyTheme, THEME_KEY); }
